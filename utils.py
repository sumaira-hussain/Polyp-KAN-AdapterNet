...
def get_network(args, net, use_gpu=True, gpu_device=0, distribution=True):
    ...
    if net == 'sam':
        ...
        options = ['default', 'vit_h', 'vit_l', 'vit_b', 'tiny_vit', 'efficientvit_l2', 'PromptGuidedDecoder',
                   'sam_vit_h']
        if args.encoder not in options:
            raise ValueError("Invalid encoder option. Please choose from: {}".format(options))
        else:
...
def vis_image(imgs, pred_masks, gt_masks, save_path, reverse=False, points=None, boxes=None):
    """
        Visualizes predicted masks, ground truth masks, and input images.

        Args:
            imgs (Tensor): Input images (B, C, H, W).
            pred_masks (Tensor): Predicted masks (B, C, H, W).
            gt_masks (Tensor): Ground truth masks (B, C, H, W).
            save_path (str): Path to save the output image.
            reverse (bool): If True, inverts the masks.
            points (Tensor or None): Optional point prompts (B, N, 2).
            boxes (Tensor or None): Optional bounding boxes (B, N, 4).
            args (Namespace or None): Optional arguments object with image_size, roi_size, out_size, thd.
        """
    ...
    # Reverse masks if required
    if reverse == True:
        pred_masks = 1 - pred_masks
        gt_masks = 1 - gt_masks
        # Resize images to match mask dimensions
        imgs = Resize((h, w))(imgs)
        if imgs.size(1) == 1:
            imgs = imgs.expand(-1, 3, -1, -1)
    # Handle multi-channel (multi-class) case
    if c == 2:  # for REFUGE multi mask output
        ...
        gt_disc, gt_cup = gt_masks[:, 0, :, :].unsqueeze(1).expand(b, 3, h, w), gt_masks[:, 1, :, :].unsqueeze(
            1).expand(b, 3, h, w)
        tup = (imgs[:row_num, :, :, :], pred_disc[:row_num, :, :, :], pred_cup[:row_num, :, :, :],
               gt_disc[:row_num, :, :, :], gt_cup[:row_num, :, :, :])
